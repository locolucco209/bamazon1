...
function thechoice() {
    ...
    if (choice === 'search') {
        connection.connect((error) => {
            if (error) throw error
            console.log('connected as ', connection.threadId)
            var sqlquery = "select * from product where product_name = " + "'" + search_item + "'";
            connection.query(sqlquery, (error, response) => {
                if (error) throw error
            ...
            connection.query(sqlquery, (error, response) => {
                ...
                prompt.get(['Quantity'], function (err, result) {
                    ...
                    if (frmuserqu <= selectedquantity) {
                        var orderquery = " update product set stock_quantity =" + (selectedquantity - frmuserqu) + " where product_name = '" + search_item + "';"
                        connection.connect((error) => {
