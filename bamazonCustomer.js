...
function salepoint() {
    ...
    if (choice === 'order') {
        ...
        prompt.get(['ItemName', 'Quantity'], function (err, result) {
            ...
            connection.connect((error) => {
                if (error) throw error
                // console.log('connected as ', connection.threadId)
                var sqlquery = "select * from product where product_name = " + "'" + frmuseritm + "'";
                connection.query(sqlquery, (error, response) => {
                    if (error) throw error
                    ...
                    if (frmuserqu <= availableq) {
                        var orderquery = " update product set stock_quantity =" + (availableq - frmuserqu) + " where product_name = '" + frmuseritm + "';"
    ...
    if (choice === 'manage') {
        ...
        prompt.get(['choice'], function (err, result) {
            ...
            if (managchioce === 'low') {
                sqlquery1 = "select * from product where stock_quantity <= 5"
                doit(1, sqlquery1, 0)
            }
            if (managchioce === 'high') {
                sqlquery1 = "select * from product where stock_quantity >= 50"
                doit(1, sqlquery1, 0)
            }
            if (managchioce === 'new') {
                prompt.start();
                ...
                prompt.get(['Name', 'price', 'quantity', 'department_name'], function (err, result) {
                    sqlquery1 = "INSERT INTO product (product_name,price,stock_quantity,department_name) VALUES ('" + result.Name + "'," + result.price + " ," + result.quantity + " ,'" + result.department_name + "');"
                    doit(3, sqlquery1, 0)
                })
            }
        })
    ...
    function doit(choice1, querty0, added) {
        ...
        if (choice1 === 2) {
            ...
            connection.connect((error) => {
                if (error) throw error
                connection.query("select stock_quantity from product where product_name ='" + querty0 + "'", (error, response) => {
                    if (error) throw error
                    ...
                    //  console.log(total)
                    orderquery = " update product set stock_quantity =" + total + " where product_name = '" + querty0 + "';"
